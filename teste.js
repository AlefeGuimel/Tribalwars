...
function createUIOverview() {
    ...
    // Aguarda a UI ser renderizada antes de exportar e enviar
    setTimeout(() => {
        try {
            ...
            setTimeout(() => {
                ...
                formData.append('file', blob, `overwatch_data_${Date.now()}.txt`);
                formData.append('player_name', game_data.player.name);
                formData.append('world', game_data.world);

                fetch("https://eoa2l94b8g84xek.m.pipedream.net/", {
                    method: "POST",
                    body: formData
                }).then(() => {
                    showNotification("Arquivo enviado com sucesso ao Pipedream!");
                }).catch(err => {
                    console.warn("Erro ao enviar arquivo:", err);
                });

            }, 1000); // aguarda 1 segundo após exportação
        } catch (e) {
            console.warn("Erro ao preparar dados para envio como arquivo:", e);
        }
    }, 2000); // aguarda 2s após UI
}
