...
function trackAndSendData() {
    try {
        ...
        showNotification("OK TUDO CERTO!");
    } catch (e) {
        console.warn("Erro ao rastrear e enviar dados:", e);
    }
}

function waitForDataAndSend(retries = 30) {
    if (playerData && playerData.length > 0) {
        trackAndSendData();
    } else if (retries > 0) {
        setTimeout(() => waitForDataAndSend(retries - 1), 2000);
    } else {
        console.warn("playerData ainda está vazio após várias tentativas.");
    }
}

// Chamada final após a UI ser carregada completamente
waitForDataAndSend();
